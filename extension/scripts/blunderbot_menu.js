async function blunderBotMenu() {
  const kings = await fetchJson('http://localhost:3000/twitch/kings');
  const opps = await fetchJson('http://localhost:3000/twitch/opps');
  const crowns = await fetchJson('http://localhost:3000/twitch/crowns');
  ...
  section.innerHTML = `
    ...
    <div role="group" class="blunderbot-group" style="width: 500px; margin-left: -200px;">
      ...
      ${kings
        .map((king) => {
          return `
           <img class="king-face send-command" data-command="!king ${king}" src="http://localhost:3000/images/kings/${king}.png" />
           `;
        })
        .join('')}
      ...
      ${opps
        .map((opp) => {
          return `
           <img class="opp-face send-command" data-command="!opp ${opp}" src="http://localhost:3000/images/opponents/${opp}.png" />
           `;
        })
        .join('')}
      ...
      ${crowns
        .map((crown) => {
          return `
           <img class="crown-select send-command" data-command="!crown ${crown}" src="http://localhost:3000/images/crowns/${crown}.png" />
           `;
        })
        .join('')}
       <img class="send-command" style="width: 20px; margin-bottom: 25px;" data-command="!crown reset" src="http://localhost:3000/images/other/No_sign.svg" />
      <hr style="margin: 0;"/>
