    ...
    colors.forEach((color) => {
      pieces.forEach((piece) => {
        if (themeConfig[theme][color][piece]) {
          styleTag.innerHTML += `
          .is2d .${piece}.${color} {
             background-image: url('http://localhost:3000/images/themes/${theme}/${color}/${piece}.png') !important;
          }
        `;
        }
      });
      ...
      pieces.forEach((piece) => {
        if (themeConfig[theme][color[0]][piece[0]]) {
          ...
          .piece.${color[1]}${piece[1]} {
             background-image: url('http://localhost:3000/images/themes/${theme}/${color[0]}/${piece[0]}.png') !important;
          }
        `;
        }
      });
    ...
    .brown .is2d cg-board::before {
      background-image: ${
        themeConfig[theme]?.boardExists
          ? `url('http://localhost:3000/images/themes/${theme}/board.png')`
...
async function setCursorStyle() {
  window.requestAnimationFrame(async () => {
    ...
    if (cursor) {
      styleTag.innerHTML += `
        cg-board {
          cursor: url('http://localhost:3000/images/cursors/${cursor}.cur'), default !important;
        }
      `;
    }
  ...
  window.requestAnimationFrame(async () => {
      ...
      styleTag.innerHTML += `
        ...
        .playing .is2d .orientation-${myColor} .king.${oppColor} {
          background-image:
            ${
              hasCrown
                ? `url('http://localhost:3000/images/themes/${theme}/${oppColor}/crown.png'),`
                : ''
            }
    ...
    if (opponent === 'buttery_flaky') {
      ...
      window.buttery_interval = setInterval(() => {
        ...
        let background_url = toggle
          ? `http://localhost:3000/images/opponents/buttery_flaky.png`
          : `http://localhost:3000/images/other/buttery_flaky2.png`;
        toggle = !toggle;
        if (swapped) {
          background_url = `http://localhost:3000/images/kings/${king}.png`;
        }
        const styleTag = document.createElement('style');
    ...
    if (opponent === 'batman') {
      ...
      window.batman_interval = setInterval(() => {
        ...
        let background_url = toggle
          ? `http://localhost:3000/images/opponents/batman.png`
          : `http://localhost:3000/images/other/bat_signal.png`;
        toggle = !toggle;
        if (swapped) {
          background_url = `http://localhost:3000/images/kings/${king}.png`;
        }
    ...
    const crownURL = crown
      ? `url('http://localhost:3000/images/crowns/${crown}.png')`
      : `url('http://localhost:3000/images/themes/${theme}/${myColor}/crown.png')`;
...
async function sendCommandByOwner(command) {
  await apiPost('http://localhost:3000/twitch/command', { command });
}
...
async function forceClientsToRefresh() {
  await apiPost('http://localhost:3000/twitch/force-clients-refresh', {});
}
...
async function translate(message) {
  const res = await apiPost('http://localhost:3000/openai/translate', {
    message
  });
